...
%
%However, we in this work do not consider a color segmentation
...
%
%In the agricultural use-case addressed in this paper, the GPU
...
%
%Nevertheless, a fluctuation in the wind's
...
considered SLAMBench \cite{nardi2015introducing}, a framework that investigates Simultaneous Localisation and Mapping (SLAM) algorithms configuration alternatives
for energy efficiency. 



Other approaches to energy modeling, such as the mission-based energy
models studied by~\cite{sadrpour2013experimental,
  sadrpour2013mission}, focus mostly on ground-based autonomous
vehicles instead of the UAVs.~\cite{morales2009power}
...
%
Energy modeling of mobile robots as carried by~\cite{mei2006deployment, mei2005case, mei2004energy} has provided
...
%of a battery shortage; to this end, we investigated 
%
To account for the eventuality of battery shortage,~\cite{berenz2012autonomous} elaborates a
battery management mission-based dynamic recharge approach by using a set of recharge stations along with
self-docking capable robots. 



%Our approach similarly allows landing on
...
%In this work we do not consider a color segmentation approach, since for a realistic (outdoor) case it would be difficult to ensure that the landing marker will be the only object of a specific color in the scene.
...
%do not consider a color segmentation
%approach, since for a realistic (outdoor)
%case it would be difficult to ensure that the
%landing marker will be the only object of a specific color in the
%scene.
...
%In our work, we use \powprof{}, a generic energy
%modeling tool~\cite{seewald2019coarse}. This tool enables measuring
%the energy impact of different configurations of the ROS-based system
%implementing the agricultural use-case.
%The \powprof{} tool is part of the TeamPlay toolchain, which aims to make tradeoffs between energy and other non-functional properties accessible to the developer. 
...
\caption{On the left is the landing marker and in the middle is the kernel that is constructed to detect the n-fold marker. The white regions, on the kernel, correspond to the real values and the black regions to the imaginary values. The grey area is of no concern. 
...
proper minimum kernel size \emph{k} based on a threshold quality score value \emph{q} thus balancing between the computationally expensive convolution process and the effective marker detection. The marker detection results under two different occlusion cases can be seen in Figure~\ref{fig:LandingMarker}.
