...
\hyphenation{analysis ex-pe-ri-men-tal}
...
for each class. We trained the CNN with a 64 batch size for 5000 iterations using default 
...
%\emph{landing marker} detection.
Landing with
%The landing time is however reduced by \SI{30}{\second} 
%when using the 
10fps update
rate is however \SI{30}{\second} faster. %The adaptive kernel size in most cases outperforms the fixed
...
the QoS level %to the available battery, 
