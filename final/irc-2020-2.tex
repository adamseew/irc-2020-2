...
marker detection and the CNN. Experiments are conducted in Gazebo simulation using an energy modeling tool to measure the computational energy cost as a function of QoS. %Our experiments 
We test the energy efficiency and robustness of our system in various dynamic wind disturbances. We show that the marker detection algorithm can be run at the highest QoS with only a marginal energy overhead whereas adapting the QoS level of CNN results in a considerable power saving. %for the system as a whole. 
The
...
%
GPS is however insufficient for robust precision landing. %, and 
...
Energy-costly computations %such as computer vision 
are adapted by
selecting the desired QoS to match the available
energy~\cite{seewald2020mechanical}. %By combining
We aim to combine energy-aware algorithm design with autonomous landing
capabilities to extend the flight time and increase the total availability of the UAV. %to
...
image from the downward facing camera into a \emph{YOLOv3-tiny} CNN\Xcite{yolov3} implemented in ROS~\cite{bjelonicYolo2018}. 
%Four
...
for each class. We trained the CNN 5000 epochs using default 
parameters and an input image size of \SI{416}{}$\times$\SI{416}{}
pixels.
...
For \emph{energy 
evaluation of tracking}, eight tests were executed for different rates 
...
%
%seen in Fig.~\ref{fig:PowerDuringTracking}. 
...
For \emph{QoS evaluation of tracking}, twelve tests were executed for different 
...
obtained for high \emph{YO} and \emph{LM} fps update rates  %\emph{YOLOv3-tiny}
% \emph{landing marker} detection,
and no wind, where 28 out of the 32
objects were detected. %This is expected since at a 
(High fps rates imply that each object is viewed more times as the UAV moves.) %, but %. Nevertheless, 
For the same high fps values but with a wind speed of \SI{12}{\meter \per \second} only 18 out of 32 objects were detected. This difference is due to the UAV %has to 
...
For the \emph{energy evaluation of landing}, four tests were executed for different 
\emph{LM} fps rates 
%\emph{landing marker} detection ROS node 
(10fps, 2fps, 1fps, 0.5fps). %under two different cases: fixed kernel size
%at \SI{22}{}$\times$\SI{22}{} pixels and adaptive kernel size.
%
...
%\emph{landing marker} detection.
The landing time is however reduced by \SI{30}{\second} when using the 10fps update
...
For the \emph{QoS evaluation of landing}, twelve tests were executed for different 
\emph{LM} fps rates 
%\emph{landing marker} detection ROS node 
(10fps, 2fps, 1fps, 0.5fps) under three different wind speeds.
...
%%   \caption{Power consumption during tracking mode. \emph{YO} fps:
...
landing time. Therefore, although the higher-QoS landing %computer vision
...
%In this paper
We have presented %a robust, energy-sensitive, 
the energy-aware design of a vision-based
algorithm for autonomous tracking and landing in varying environmental
conditions, based on experiments %ly executing %all the necessary the algorithm on the 
...
energy overhead, whereas adapting the QoS level of the CNN %\emph{YO}
%\emph{YOLOv3-tiny} CNN
results in a considerable power saving for the system as a whole, 
%with a significant power saving for a %system executing on
