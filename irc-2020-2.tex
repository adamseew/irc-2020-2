...
perform all computations on-board using an \emph{Nvidia Jetson Nano} and analyse %we attempt to
...
Experiments are conducted in Gazebo simulation using the \emph{powprofiler} energy
modeling tool to measure the energy cost as a function of Quality of Service (QoS). %for autonomous
...
fixed-wind UAV but only marginal if executing on a standard multirotor UAV.

% [Ad2:] Typo: In this paper, [grammarly does this...]
% [Ad2:] Typo: onboard
% [Ad2:] Typo: fixed-wing
% [Ad2:] Since the reader has no idea what powprofiler is at this point, I suggest using 'an energy modeling tool' instead.
% [Ad2:] Quality of Service (QoS) why to explain the acronym; I suppose it should be just QoS while it's properly introduced at first occurrence in the Introduction
% [Ad2:] Various dynamic wind disturbances sounds confusing; I guess we might use dynamic only.
...
% [Ad2:] Typo: in this paper,
% [Ad2:] Typo: landing system and
...
% [Ad2:] NVIDIA here but Nvidia in abstract {I guess it should be NVIDIA}
...
% [Ad2:] Saripalli demonstrated sounds almost like he did some formal demonstration of math-like kind of the Kalman filter variation he used. What about just used?
% [Ad2:] I don't like that the paragraph is not self contained; it adds some information to Saripalli but goes directly to Lee. What about moving Saripalli...offline to the above paragraph?
...
% [Ad2:] Typo: if you don't stick with the above suggestion, there is a comma after However missing
% [Ad2:] Typo: onboard
...
% [Ad2:] reader learns about markers being of different shapes and colors, so s/he might ask how specific. I think we could just omit the sentence "Here a specific landing marker was used and a specific CNN was used to detect it".
% [Ad2:] as a landing marker -> for landing
% [Ad2:] I would omit this information, it doesn't add much about the approach "for the purpose of ``2016 DJI Developer Challenge''".
% [Georgios2:] Agree on that. Removed the information.

% [Ad2:] teams -> approaches {here is really unclear if it refers to the participants of the challenge or the approaches}
...
% [Ad2:] have completed -> have been completed
...
% [Ad2:] described -> used {It's described in this paper exactly here; so I would avoid; seems like an infinite loop}
...
% [Ad2:] energy-sensitive design approach doesn't seem to fit the context. Maybe point of view instead of approach
...
% [Ad2:] Typo: computationally
% [Georgios2:] Changed "computational" to "computationally".

% [Ad2:] Typo {the comma typo}: However,
%[Georgios2:] Added the ",".

% [Ad2:] on the selected -> in the selected
...
% [Ad2:] it should be NED coordinates? Frame is a navigation frame where we'd have a world one that defines the origin and the body one that define the position of the UAV?
...
% [Ad2:] will be used sounds like future work. Perhaps is being used?
%[Georgios2:] Agree on that. Changed from "will be used" to "are used"

% [Ad2:] the altitude of the UAV and the attitude of the UAV (roll,pitch,yaw) -> the altitude and the attitude (roll, pitch, and yaw angles) of the UAV
...
% [Ad2:] a suggestion for next one. Not now as probably we'd have to change a lot: \mathcal{O}_B for body frame, \mathcal{O}_something for some navigation frame; cooler and simpler
...
% [Ad2:] BODY frame {unless there is a valid reason for BODY pose}
...
% [Ad2:] The world coordinates -> the coordinates {coordinates in respect of the world reference frame and not world's coordinates}
...
% [Ad2:] The pixel coordinates are converted into world coordinates -> Pixels are converted into meters {not sure what pixel coordinates are as this is the first time we mention them}
...
% [Ad2:] The world coordinates -> the coordinates {coordinates in respect of the world reference frame and not world's coordinates}
% [Ad2:] are converted from the camera's \emph{BODY} frame into the camera's \emph{NED} frame -> are converted to the \emph{NED} coordinates {NED is not a frame, NED are coordinates or I am totally missing something here}
...
% [Ad2:] The world coordinates -> the coordinates {coordinates in respect of the world reference frame and not world's coordinates}
% [Ad2:] are converted from camera's \emph{NED} frame -> are converted from \emph{NED} coordinates {NED is not a frame, NED are coordinates or I am totally missing something here}
...
% [Ad2:] NED coordinates
% [Ad2:] or ... velocities ->   {How to differentiate velocities? If the velocity is from the flight controller, it's just a value. So we calculate the tangent? I would omit this. Later we don't mention any differentiation but just say they are 'obtained from the flight controller's onboard sensors'}
...
% [Ad2:] it's confusing when we use EKF ourselves *since we have a discrete KF...* and when the EKF from PX4 [I am confused so I can immagine the reader...]
% [Ad2:] will be used -> was used {not sure I'd go with future, as normally we use past}
...
\begin{equation}
  \tilde{x}_t^{\text{obs}}:=\bar{\dot{x}}_t\,d_t^{\text{obs}}, %lol why not->|{\hat{x}_t-\hat{x}_v}|\text{ with }v:=t-d_t^{\text{obs}}, seems you loose the amplitude of the velocity???
\end{equation}
...
\begin{equation}
    ...
    \hat{x}_{t}^f = \hat{x}_{t} + K_{t}(x^{\text{obs}}_t+\tilde{x}^{\text{obs}}_t - \hat{x}_{t}), \\ %<- TBH not entirely sure why there as there is no system but directly estimate (you would compare the system with the measurement at this step) but ok; if it works that I am fine with it :) [just confirm it does]
...
=======
% [Ad2]: P_{t} = ... P_{t} {last line should be P_{t} before and after estimation. Use P_{t}^- (what textbook? I can check the notation; Hector? Then P_{t}^f = ... P_(t}))}

% [Georgios2:] That is true. We can use the symbols for "before" and "after" the correction step considering not only the convariance matrix P but also the x_hat ... fell free to change them accordingly
...
dx_{obs} = vx_{mean_{obs}} * dt_{obs} \quad , \quad
    ...
    \end{array}
\end{equation*}
>>>>>>> 94dee7f3b1b4db6e79b34a069dd98074d5c7c5fd
...
% [Ad2:] big issue here! One reads that we differentiate to estimate the velocity (dx/dt if x is the position). Then one reads that we use low pass filter to estimate the velocity. So which one? Confusing... Maybe omitting and just saying we also you low pass filter due to possible oscillations of the UAV?
...
%\cite{Dynamic_pressure_NASA,anderson2010fundamentals}: <- let's use the textbook only (why the website). We have too many references anyways...
\begin{equation}
F_t = \frac{\varrho  v_t^2}{2}   A,
\end{equation}
where $F_t\in\mathbb{R}$ is the force keeping the altitude constant, 
%\(p_{d}\) is the dynamic pressure, 
\(A\in\mathbb{R}_{\geq 0}\) is the area of the applied pressure, \(\varrho\in\mathbb{R}_{\geq 0}\) is the density of the
air %(around \SI{1.2}{\kg \per \m^3}) <- I hope the reader knows/[can google] this :)
, and \(v_t\in\mathbb{R}\) is the wind velocity in its two components at time $t$. %\SI{}{\m \per \s}. why; it would work also with other measures...
...
%    \item A random float number between 0.9 and 1.2 is chosen at every update cycle. This number is defined as \emph{Random\_noise}.%<-then we define it later. Redundant
    
    \item The applied wind force is calculated $F_{t+1}=\|w\cdot x\|F_t*r$ 
%<- so complex for such an easy equation... 
%<- If you apply norm on just one component it's absolute value?
%    \\
%    
%  
%    
...
%\(x_{norm } = 0.8\), \(y_{norm}  = 0.2\) , \(\mbox{\em power}_{init} = 7.0 N\) <- wait if this is constant then you get always the same value plus some random stuff; let's write it so it doesn't look constant :)
...
% [Ad2:] I am confused. Seems power and force are the same and if it's not the case then I don't understand what is going on (why would you calculate the force if you don't use it and then you use power instead). Correct me if I didn't understand
   
   \item If instead the altitude $h$ is $h\leq 3$, the force is constant $F^h_{t+1}=1/2$ N.%For a UAV altitude of \SI{3}{\meter} and below, \(\mbox{\em power} = 0.5\). 
