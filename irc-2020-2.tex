...
% [Ad3:]\emph in the abstract? Eventually put it back (it was \emph{NVIDIA Jetson Nano})
...
% [Ad3:] then it's not consistent with formatting (used modeling tool instead of powp\emph{powprofiler}rofiler)
modeling tool to measure the energy cost as a function of QoS. %for autonomous
% [Ad3:] acronym shouldn't be explained in the abstract but at first occurrence in the paper (I mean you can do it but then I'd expect the same for CNN and UAV)
...
% X [Ad2:] Typo: in this paper, [Ad3:] oko this doesn't sound that good after the comma anyways
...
% [Ad3:] Good, it's another Saripalli's work; thought the previous chapter was the same work (hence the confusion). Maybe later instead of also? Flows sort of better.
...
% [Ad3:] perhaps vision measurement -> distance measurement? Is vision measurement a conventional way of saying the value of the distance taken from the lidar? IDN
...
% [Ad3:] LOL sorry for being annoying but it's et. al. in both cases so it's not entirely clear which researchers from the et. al. :P
...
% [Ad3:] yes, but it sounds we are criticizing them while we are not even stating in that sentence what we are using instead (if I am a reviewer and I see somebody taking such position I start questioning). I would be rather conservative. Maybe omitting However on?
...
% [Ad3:] In the sentence Here...detect it we might change one of the specific to given (3x specific like words)
...
% [Ad3:!] we are just estimating the position of the drone given the velocity of the moving platform.
...
%
% [Ad3:] Ulrik, the sentence seems to fit better in SoA?
%
...
% [Ad3:] of the experiment -> of the exeprimental? Sounds sort of better. 
...
% [Ad3:] what we do is actually regression because we use gnuplot; while if we use interpolation we have to explicitly call it in powprof. I guess fine then; just being very very annoying :)
...
% [Ad3:] the thing about approach is that we say it earlier meaning 'the paper'. Hence confusion (the paper is not only energy sensitiveness)
% [Ad3:] it's not the use-case being realistic but the simulation? 'realistic use-case' -> 'realistic simulation'
...
% [Ad3:!] what does it mean different ids?
...
output a \SI{640}{}$\times$\SI{480}{} pixel image at \SI{10}{} frames per second (fps).

% [Ad3:] I'd put the number in the text as it is, but we started with this SI earlier, so now I am just being consistent.
...
% [Ad3:] oki doki [but 'the simplest way to explain it' is very definitive. Are you entirely entirely sure of it? ;P]
...
% [Ad3:] here is exactly where putting coordinate frame instead of just frame would just make it
%
...
% [Ad3:] two instead of 2 (otherwise I get confused if to use SI)
...
% [Ad3:] confusing sry ;( I understood you read the position from PX4 in NED coordinate frame. I read UAV frame has it's own coordinate frame which is (0,0,0) that is converted into NED coordinate frame: you just use the reading right? It's not a conversion then
...
% [Ad3:] state -> position; state seems confusing with battery, compoenents
% [Ad3:] sentence 'This is important because it will allow us to process images at different fps according to a desired QoS.' mmmh; not sure. Not really actually because we don't change QoS dynamically with the position (we actually don't change QoS at all)
...
$x_k$, \todo[noinline]{I think we might have used a bad name for this value. It currently overlaps with the estimated position of the UAV
relative to the landing target.}
calculated as described in Section~\ref{sec:Detection}.

% [Ad3:] confusing; we calculate the position here of the UAV but they are calculated as described in another section. Would erase calculated on, or describe that one is the visual measurement and is used for the purpose of estimation in our EKF.
...
\item the acceleration of the UAV, $\ddot{x}_k$, in \emph{NED} frame, obtained 
either from the flight controller's EKF or by differentiating the velocities.\todo[noinline]{These values are also described on the next page.}
...
% [Ad3:] it's little confusing because ... well "which one are you using guys?"; I'd omit still ¯\_(ツ)_/¯ 
...
\todo[disable]{Not comments but todos here (for Henrik) %¯\\_(ツ)_/¯

[Ad3:] the math here is really unecessarily complicated >:P . 
I can change it the robo-gang way. Your formatting of where up is 
definitely interesting; when I started writing papers with math 
I also had this thing of writing my way. 
Then came Hector one day and told me that only once I am known enough 
I can change the status quo in the math notation; 
so now I am little autistic about it all the time :)

[Ad3:] there are fixes with ts here and there. 
Also; you have a time dependent timestep dt. Please, don't use dt. 
What is that? It seems differentiation. Is soooo confusing. 
I can help to simplify. Just tell me!}

where $\hat{x}_k \in \mathbb{R}^2$ is the position of the UAV, 
and $F$ is a 2 by 2 identity matrix.
The input matrix and the associated control input is given by:
\begin{align*}
G_{k} & = \begin{bmatrix}
...
u_{k} & = \begin{bmatrix} 
                ...
                \end{bmatrix}^\top
\end{align*}
The initial covariance matrix $P_0$ is a 2 by 2 unit matrix
and the process noise $Q_k$ \todo[noinline]{Is this the right word?} 
is a 2 by 2 unit matrix multiplied with 
$\Delta t_k \cdot \sigma^{2}_{\textrm{IMU}}$.
...
\begin{align}    
    ...
    S_{k} & = H_{k} \cdot P_{k} \cdot H_{k}^\top + R_{k} \\
    K_{k} & = P_{k} \cdot H_{k}^\top \cdot S_{k}^{-1} \\
    \hat{x}_{k} & = \hat{x}_{k} + K_{k} \cdot e_{k} \\
...
\begin{align*}
  ...
  R_{k} = \begin{bmatrix} 
            1 & 0 \\ 
            0 & 1
            \end{bmatrix} \cdot \sigma^{2}_{obs}  \\ 
  dx_{obs} & = vx_{mean_{obs}} \cdot dt_{obs}, \qquad
  dy_{obs} = vy_{mean_{obs}} \cdot dt_{obs} 
\end{align*}
