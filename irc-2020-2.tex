...
prediction for the state of the UAV at any given time. Such a prediction allow us potentially to process images at different fps
...
$u_{k} = \begin{bmatrix}\dot{x}_k & \ddot{x}_k & \dot{x}_{k}^l\end{bmatrix}^\top$ is its control with $x_k,x_k^l$ being the positions of the UAV and the moving paltform respectively.
...
%}
\noindent where $F_{\text{init}}$ is the force, \(p_{d}\) is the dynamic pressure, \(A\) is
...
\begin{itemize}
    \item For an altitude of \SI{6.0}{\meter} and above, $F=F_{\text{init}}$. % where $F_{\text{init}}$ is either \SI{3.45}{\newton} or \SI{7.76}{\newton}
% [Ad4:] said before.
...
based on the Gazebo models. The dataset consists of \SI{1200}{} images, \SI{300}{}
for each class. We trained the \emph{YOLOv3-tiny} by its default training
parameters for \SI{5000}{} epochs, for an input image size of \SI{416}{}$\times$\SI{416}{}
pixels.
