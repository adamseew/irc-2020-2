...
%where:
\begin{itemize}
\item  \(x, y \) is the position of the UAV in the landing site's local coordinate system (aligned with NED frame), 
\item \(dt\) is the time interval the PX4's EKF outputs the \(v_x, v_y\) data, usually around 33ms, 
\item \(v_x, v_y\) is the linear velocity of the UAV in NED frame, estimated from the PX4's EKF,

% [Ad2:] NED frame -> NED coordinates

\item \(a_x, a_y\) is the linear acceleration of the UAV in NED frame, estimated on the companion computer from differentiating the velocities.

% [Ad2:] NED frame -> NED coordinates

\item \(v_{x,\textrm{landing}}, v_{y,\textrm{landing}}\) is the linear velocity of the moving platform in NED frame, estimated from the velocity estimator for the moving platform,

% [Ad2:] NED frame -> NED coordinates

\item \(\sigma^{2}_{\textrm{IMU}}\) is the variance of the velocities based estimated by the PX4's EKF.
\end{itemize}
...
is calculated by differentiating two sequential \emph{X, Y} positions of the tractor, obtained by detecting the n-fold marker from the vertically facing camera as explained in section \ref{sec:Detection}.  \todo[noinline]{Where do these position measurements come from? the tractor gps?}
