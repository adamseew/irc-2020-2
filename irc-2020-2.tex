...
%, and is achieved in simulation iteratively in three steps. 
The energy-sensitive design is achieved in three steps.
%
First, the developer specifies the maximum and minimum QoS level for each algorithm running on the system. %that the use-case is composed of. 
...
%In section \ref{sec:Detection} the computer vision algorithms for detecting the landing marker along with the algorithms used to measure a relative position of the UAV from the moving platform will be presented. In section \ref{sec:Navigation} the sensor fusion algorithms used to estimate the UAV's relative position from the moving platform along with the moving platform's velocity will be presented. In section \ref{sec:Guidance} the algorithms used for UAV's path-planning in 3D space along with the calculation of its desired velocities will be demonstrated. In section \ref{Control} the algorithms used to control the UAV accordingly will be presented.
...
% The full process can be described as:
%
%\begin{equation}
%    \begin{array}{l}
%
%    vx_{m_{NED(t)}} = 
%    
%    k_{1}*(( \frac{X_{(t)} - X_{(t-1)}}{dt}) + vx_{NED_{mean}}) + k_{2} * vx_{m_{NED(t-1)}}
%    
%    \\
%    \\
%
%     vy_{m_{NED(t)}} = 
%    
%    k_{1}*(( \frac{Y_{(t)} - Y_{(t-1)}}{dt}) + vy_{NED_{mean}}) + k_{2} * vy_{m_{NED(t-1)}}
%    
%    \\
%    \\
%    
%  \Vert v_{m_{NED(t)}} \Vert  = \sqrt{vx_{m_{NED(t)}}^2 + vy_{m_{NED(t)}}^2}
%          
%    \end{array}
%\end{equation}
%
...
%\begin{itemize}
%
%\item  \(vx_{m_{NED(t)}}, vy_{m_{NED(t)}} \) is the velocity of the moving platform at time \(t\) in \(x,y\) NED frame, 
%
%\item  \(X_{(t)}, Y_{(t)} \) is the measurement of the moving platform's position at time \(t\) in NED frame, 
%
%\item \(dt\) is the time interval between two position measurements at time \(t\) and \((t-1)\),
% 
%\item \(vx_{NED_{mean}},vy_{NED_{mean}}\) is the mean linear velocity of the UAV in NED frame between two position measurements at time \(t\) and \((t-1)\), estimated from the PX4's EKF,
%
%\item \(k_{1}, k_{2}\) are the gains of the low-pass filter and \( k_{1} + k_{2} = 1.0 \). For a low-pass filter \(k_{1} < k_{2} \).
%
%\item \( \Vert v_{m_{NED(t)}} \Vert \) is the magnitude of the moving platform's velocity.
%
%\end{itemize}
