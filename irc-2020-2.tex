...
Given $I:=I_2$, a \SI{2}{}$\times$\SI{2}{} identity matrix, the matrix $F=I$, the noise covariance matrix $Q_k=\Delta t_k\cdot\sigma^2_{\textrm{IMU}}\cdot I$, and $\sigma^2_{\textrm{IMU}}\in\mathbb{R}$ is the variance of the velocities retrieved from the flight controller's data.\todo[noinline]{Is this the right word? [Ad3:] actually no/yes--diatrabes between controls people stating that the model of the system is perfect since is a model [cite Hector]} Further, \(\Delta t_k\) is the time interval the flight controller output the data (usually around \SI{33}{\ms}). 
The input matrix is given by:
\todo{Henrik, little lengthy with implicitly saying "is a 2 by 2 identity matrix"; what about this one?}
\begin{align*}
G_{k} & = \begin{bmatrix}
...
\end{align*}
\todo{
  [Ad3:] Actually $G_k$ might still go easily inline. Something like $G_k=\Delta t_k\begin{bmatrix}I & \Delta t_kI/2 & -I\end{bmatrix}$. Up to you}
...
\begin{itemize}
    ...
    \todo{
          [Ad3:] this is just $F_k=w\cdot F\cdot r$? where $w,r$ is the direction vector you defined earlier \emph{[0.8, 0.2]} and $r$ a random number.
        } 
