...
@misc{Dynamic_pressure_wikipedia,
    title = "{Dynamic pressure (wikipedia)}",
    author="{wikipedia}",
    year = "2020",
    howpublished = "\url{https://en.wikipedia.org/wiki/Dynamic_pressure}"
}
...
The next step is to convert these pixel coordinates into a real world relative position $[X,Y,Z]$ according to a local coordinate frame. The origin of the local coordinate frame $[0,0,0]$ is defined as the center of the landing marker and alignment according to the \emph{NED} frame. %UPS: @Georgios I'm not sure I understand what the NED frame is, can you rephrase or elaborate?
%
...
The magnitude of the applied force, corresponding to a certain wind speed is calculated by the following equations \cite{Dynamic_pressure_NASA,Dynamic_pressure_wikipedia}:
